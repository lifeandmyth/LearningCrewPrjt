...
def search_news_by_keyword(request, keyword):
    ...
    # # 연관어 목록 불러오기
    # if keyword == '프로그래밍':
    #     related_words = RelatedKeywords.objects.filter(words__icontains='프로그래밍').values_list()
    # elif keyword == '투자':
    #     related_words = RelatedKeywords.objects.filter(words__icontains='투자').values_list()
    # else:
    #     related_words = RelatedKeywords.objects.filter(words__icontains='서비스').values_list()
