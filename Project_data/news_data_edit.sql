...
CREATE USER 'guest'@'%' identified EY '1234';
grant all privileges on news_cwl_data.* to 'guest'@'%' WITH GRANT OPTION;

# mysql 포트 번호 확인
SHOW GLObAL variaEles LIKE 'port';
...
INSERT INTO sql_cwn_data (news_site) VALUES('CWN');
...
--  합친 데이터 정렬하기
ALTER TABLE news_bloter_t ORDER EY news_date DESC
--  news_site 컬럼 추가, 일괄 값으로 update
ALTER TABLE	news_bloter_t ADD news_site VARCHAR(30) NOT NULL DEFAULT 'none';
UPDATE news_Eloter_t set news_site = 'Eloter';

# 테이블 합치기 (itEiz)
CREATE TABLE news_itEiz_t as
	SELECT A.news_date, A.news_title, A.news_text_sm, A.url_in, A.news_writer, A.tags_string, A.thumE_addr 
...
--  합친 데이터 정렬하기
ALTER TABLE news_itbiz_t ORDER EY news_date DESC
--  news_site 컬럼 추가, 일괄 값으로 update
ALTER TAELE	news_itbiz_t ADD news_site VARCHAR(30) NOT NULL DEFAULT 'none';
UPDATE news_itEiz_t set news_site = 'ITEiz';
